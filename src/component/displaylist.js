...
function DisplayList(props) {
    /*
        get the firstKey 
    */
    let gistKey = Object.keys(props.userGistList[0])[0];
    return (
            ...
            <div className="grid-item fileNameMiddle">
                {
                    props.userGistList.length > 0 && props.userGistList.map((eachItem, idx) => {
                        let key = Object.keys(eachItem)[0]
                        return (
                            Object.keys(eachItem[key].files).map((eachFile) => {
                                let eachFileType = eachItem[key].files[eachFile].language
                                let eachFileName = eachItem[key].files[eachFile].filename
                                return (
            ...
            <div className="grid-item forkNameLast">
                {
                    props.perGistsFork.length > 0 && props.perGistsFork.map((eachFork) => {
                        let key = Object.keys(eachFork)[0]
                        let eachGistForDetails = eachFork[key]
                        return (
